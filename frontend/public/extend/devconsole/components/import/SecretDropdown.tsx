...
class SecretDropdown extends React.Component<LabelDropdownProps, LabelDropdownState> {
  readonly state = {
    items: {},
  ...
  componentWillReceiveProps(nextProps: LabelDropdownProps) {
    const { authTypeSelector, resources, loaded, loadError, placeholder } = nextProps;
    if (!loaded) {
    ...
    _.each(resources, ({ data }) => {
      _.reduce(
        ...
        (acc, resource) => {
          if (
            resource &&
            resource.hasOwnProperty(authTypeSelector) &&
            (resource[authTypeSelector] === 'kubernetes.io/basic-auth' ||
              resource[authTypeSelector] === 'kubernetes.io/ssh-auth')
          ) {
